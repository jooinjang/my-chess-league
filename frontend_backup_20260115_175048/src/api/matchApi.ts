...
export const matchApi = {
  ...
  getAnalyzedMatchIds: async (): Promise<number[]> => {
    const response = await apiClient.get<ApiResponse<{ match_ids: number[] }>>('/analysis/matches');
    return response.data.data?.match_ids || [];
  },
};
