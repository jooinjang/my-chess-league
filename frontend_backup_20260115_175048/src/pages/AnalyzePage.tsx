...
export function AnalyzePage() {
  ...
  const urlMatchId = matchIdParam ? parseInt(matchIdParam, 10) : null;
  ...
  // Load existing analysis for a match
  const loadExistingAnalysis = useCallback(async (matchId: number) => {
    setLoadingAnalysis(true);
    try {
      const result = await matchApi.getAnalysis(matchId);
      ...
      // Analysis doesn't exist - this is expected for unanalyzed matches
      setHasExistingAnalysis(false);
    } finally {
  ...
  // Auto-load match from URL parameter
  useEffect(() => {
    if (urlMatchId && matches.length > 0 && !selectedMatch) {
      const match = matches.find((m) => m.id === urlMatchId);
      if (match?.pgn) {
        // Load the match
        ...
        if (success) {
          ...
          loadExistingAnalysis(urlMatchId);
        }
      }
    }
  }, [urlMatchId, matches, selectedMatch, loadPgn, loadExistingAnalysis]);
  ...
  const handleSelectMatch = (matchId: number) => {
    ...
    if (match && match.pgn) {
      ...
      // Sync URL if not already matching
      if (urlMatchId !== matchId) {
        navigate(`/analyze/${matchId}`, { replace: true });
      }

      // Try to load existing analysis
      loadExistingAnalysis(matchId);
    }
  };
  ...
  return (
    <div className="analyze-page">
      {/* Show back button and match info when accessed via URL */}
      {urlMatchId && selectedMatch ? (
        <div className="match-header">
          ...
          <div className="match-title">
            <h1>{selectedMatch.white_player?.name || 'White'} vs {selectedMatch.black_player?.name || 'Black'}</h1>
            <span className="match-date">{formatMatchDate(selectedMatch.played_at)}</span>
            {hasExistingAnalysis && (
      ...
      {!selectedMatch && !loadingAnalysis && (
        <div className="no-game-selected">
          <p>{urlMatchId ? 'Match not found or has no PGN data.' : 'Select a game from the dropdown above to start analyzing.'}</p>
        </div>
      )}
    </div>
  );
