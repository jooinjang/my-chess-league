...
export function MatchesPage() {
  ...
  const [analyzedMatchIds, setAnalyzedMatchIds] = useState<Set<number>>(new Set());
  ...
  const loadData = async () => {
    try {
      const [matchesData, usersData, analyzedIds] = await Promise.all([
        matchApi.getAll(),
        userApi.getAll(),
        matchApi.getAnalyzedMatchIds(),
      ]);
      setMatches(matchesData);
      setUsers(usersData);
      setAnalyzedMatchIds(new Set(analyzedIds));
    } catch (error) {
  ...
  return (
    <div className="matches-page">
      ...
      <MatchList matches={matches} analyzedMatchIds={analyzedMatchIds} onDelete={handleDeleteMatch} />
    </div>
  );
}
